...
浏览器打开 `http://127.0.0.1:8000`。前端采用高信息密度的深色研究终端布局：左侧固定工作区与数据连接状态，顶部显示行情日、构建编号和数据模式，主区以候选表格 + 右侧研究详情为核心，并保留研报库、构建历史和重新构建功能。前瞻验证结果也直接接入终端，可查看不同候选状态在 T+5/T+20/T+60 的成熟样本、平均/中位收益、正收益率和相对首个配置基准的超额收益。前端拆分为 `web/index.html`、`web/styles.css` 和 `web/app.js`，避免继续在单文件中叠加样式覆盖。
